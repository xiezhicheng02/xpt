...
func (s *DHTService) handleQueryMessage(addr *net.UDPAddr, node *bencode.BNode) {
...
func sendPingResponse(addr *net.UDPAddr, node *bencode.BNode, s *DHTService) {
	t := node.GetString("t")
...
func upsertDhtNode(addr *net.UDPAddr, node *bencode.BNode, s *DHTService) {
	a := node.Get("a")
